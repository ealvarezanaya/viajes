...
.factory('Chats', function($cordovaSQLite) {
  ...
  return {
    all: function() {
      ...
      $cordovaSQLite.execute(db, 'SELECT * FROM viajes ORDER BY id DESC')
       .then(
          function(result) {
             if (result.rows.length > 0) {
                      ...
                      { 
                        chats.push({"id":result.rows.item(i).id,
                                    ...
                                    "fecha_inicio":result.rows.item(i).fecha_inicio,
                                    "fecha_regreso":result.rows.item(i).fecha_regreso,
    ...
    remove: function(chat) {
      $cordovaSQLite.execute(db, 'DELETE FROM viajes where id = ?',[chat.id])
    ...
    get: function(chatId) {
        ...
        $cordovaSQLite.execute(db, 'SELECT * FROM viajes where id = ?',[chatId])
       .then(
          function(result) {
             ...
             if (result.rows.length > 0) {
                        chats.push({"id":result.rows.item(0).id,
                                    ...
                                    "fecha_inicio":result.rows.item(0).fecha_inicio,
                                    "fecha_regreso":result.rows.item(0).fecha_regreso,
